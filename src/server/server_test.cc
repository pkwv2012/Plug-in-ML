...
class TestServer : public Server {
 public:
  ...
  void TestServePull(int32, const Message_RequestMessage &);
  void TestServePush(int32, const Message_RequestMessage &);
  bool TestInitialize(int32);
  void TestStart(std::queue<std::string>*);
};

bool TestServer::TestInitialize(int32 bound) {
...
void TestServer::TestServePush(int32 sender_id,
...
void TestServer::TestServePull(int32 sender_id,
  const Message_RequestMessage &request) {
...
std::string Generate(int32 from, int32 type, int32 length,
  int32 index[], float value[] = NULL) {
  std::string msg_str;
  if (type == 0) {
