...
void Fifo::Open() {
  int flag = is_reader_ ? O_RDONLY : O_CREAT | O_RWONLY;
  fd_ = open(filename, flag, S_IREAD | S_IWRITE);
  if (fd_ == -1) {
...
void Fifo::Wait() {
  ...
  int sig = 0;
  int size = read(fd_, reinterpret_cast<char*>(&sig), sizeof(int));
  CHECK_NE(size, -1);
}
...
void Fifo::Signal() {
  ...
  int sig = 0;
  int size = write(fd_, reinterpret_cast<char*>(&sig), sizeof(int));
  CHECK_NE(size, -1);
}
